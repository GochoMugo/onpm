...
function writePackageJSON(directoryPath, object, callback) {
  ...
  fs.writeFile(path.join(directoryPath, "package.json"), function(error) {
...
*/
function savePackage(directoryPath, key, name, version, callback) {
  var obj = readPackageJSON(directoryPath);
  if (obj === null) {return callback()};
  obj[key] = obj[key] || {};
  obj[key][name] = "^" + version;
  writePackageJSON(directoryPath, obj, callback);
}
...
exports.installFromCache = function(name, version, options, callback) {
  ...
  getPackagePath(name, version, function(err, packagePath) {
    ...
    copyDirectory(packagePath, destPath, function(err) {
      ...
      savePackage(destPath, options.type, name, version, callback);
    });
  });
};
