...
function copyDirectory(source, destination, callback) {
  debug("making directory, if doesn't exist: %s", destination);
  // copying the source directories to destination
  debug("copying from %s to %s", source, destination);
  ncp(source, destination, {stopOnErr: true}, function(err) {
    if (! err) {
      // successful copying
      debug("successful copy");
      return callback();
    } else {
      // cleaning up
      debug("cleaning up, removing: %s", destination);
...
exports.storeIntoCache = function(name, sourcePath, callback) {
  if (typeof sourcePath === "function") {
    callback = sourcePath;
    sourcePath = path.join(__dirname, "node_modules", name);
  }
